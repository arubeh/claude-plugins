---
name: ui-test
description: テスト仕様書に基づきブラウザ MCP（browser_* ツール）でブラウザを直接操作して UI テストを実行し、判定結果をレポート出力する。
---

# UI テストスキル

テスト仕様書（Markdown）を入力として受け取り、**ブラウザ MCP（Playwright MCP 互換の `browser_*` ツール）経由でブラウザを直接操作**して UI の表示・動作を検証します。コードは生成しません。各テストケースの PASS/FAIL を判定し、スクリーンショット付きのレポートを出力します。

## 前提条件

ブラウザ MCP（`browser_*` ツール）が有効であること（本プラグイン acdp-browser が提供。
acdp バイナリ不在の環境では 0 tools になる）。ツール一覧に `browser_*` が見えない場合は、
本スキルは実行できないため acdp バイナリの導入（arubeh-installer）を案内して中断する。
...
- `cdp.log`: `browser_cdp_log` を呼んだ時のみ生成。そのステップでブラウザ MCP が送受信した CDP メッセージのダンプ (debug 用途)
...
`filename` を受け取るすべてのツール (`browser_take_screenshot` / `browser_snapshot` / `browser_cdp_log`) は、**渡された文字列をそのまま `test-results/` 配下に書き出す** (相対パスの自動プレフィックス)。つまり LLM 側で **シナリオ名 + 時刻 + ステップ番号 + 固定ファイル名** を連結したフルパスを毎回渡す必要がある。
...
**コード生成は行わない。** ブラウザ MCP の `browser_*` ツールでブラウザを直接操作する:
...
6. **エラー記録** — FAIL/ERROR の場合、同じフォルダに `error.txt` を生成。`browser_set_artifact_dir` を先に呼んでおけば、ブラウザ MCP の自動 dump が同じフォルダに `cdp-error-*.md` を残すので、`error.txt` 内でそのファイルを参照できる

### 原則: semantic-first、snapshot は discovery 用のみ（重要）

**`browser_*_by_name` を第 1 選択として使う。** CDP `Accessibility.queryAXTree` を直接叩いて要素を取得する拡張ツール群で、以下の 3 問題を同時に解消する:
...
**ファイル名はステップフォルダのフルパスを含める** — 例えば `"stories/suumo-search-meguro-mansion/20260411-200439/04/snap.md"`。こうしないと `test-results/` 直下にフラットに散らばって、後からどのステップの snap か分からなくなる。渡された相対パスは自動的に `test-results/` 配下に書き出されるので、上記のパスは `test-results/stories/suumo-search-meguro-mansion/20260411-200439/04/snap.md` に着地する。
...
以下は acode テンプレート採用プロジェクトの場合（該当スキルが無い PJ では無視してよい）:
