...
## 参加判定（インストール＝オプトイン、未参加 PJ では no-op）

arag-memory と同じ原則。以下を**すべて**満たすとき有効、1 つでも欠けたら全フック no-op / fail-open:

- `cgc` バイナリが PATH または `~/.acode/bin/` に存在する
- プロジェクトに `.cgc/graph.json` が存在する（= `cgc index .` 実行済み）
- `.cgc-disabled` ファイルが**存在しない**（明示オプトアウト）

未参加 PJ での参加方法は `cgc index .` を 1 回実行するだけ（`.cgc/` が生成され次セッションから有効）。
...
- [x] 未参加 PJ（`.cgc/` 無し）で全フックが無言 no-op（gate / session-start / MCP は inactive 空サーバ応答）
