...
capture は一時ファイルでなく **`./.arag/_captured/<date>_<sid>_<i>_<slug>.md` に永続書き出し**（§7 の **raw アーカイブ**・監査用）したうえで、**`arag add-text --id <ファイル名base> --source <出典> --metadata <JSON> -`** で取り込む（v0.2.0 / arag#66）。frontmatter は本文に入れず metadata で渡すためノイズチャンク化せず、id 付きのため再取込は upsert、`source` には Issue/PR 等の本来の出典が載る（arag 側で added_at / channel / added_by / host も自動記録）。global 昇格は同項目を `--project _global` で add-text する。**旧 arag（add-text 非対応）では従来の `arag add` へ自動フォールバック**する。
